...
def extract_cell_id(livestream_url):
    """
    Extract camera/cell ID from URL like:
    ...&stream=camera_167
    """
    match = re.search(r"camera_(\d+)", livestream_url)
    if match is None:
        return "unknown"
...
class Simulator:
    ...
    def run_experiment(self, tf, dt, controller=None, experiment_type="DoublePendulum", user_token=None, x0=None, preparation_time=0.0, record=False):
        ...
        cell_id = extract_cell_id(self.live_url)
        ...
        while meas_time < tf and i < n:
            ...
            if i == 0:
                ...
                # Control logic
            if controller is not None:
                # tau = controller.get_control_output(x=self.x, t=meas_time, cell_id=cell_id)
        ...
        # ---------------------------------------------------
        # Keep only the samples that were actually collected
        # ---------------------------------------------------
        ...
        print("Final measured time:", self.t_values[-1])
        
        return self.t_values, self.x_values, self.tau_values, self.des_tau_values, self.vod_filepath, cell_id
