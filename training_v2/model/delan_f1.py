...
# ---------------------------------------------------------------------------
# Helpers (verbatim from Lutter's repo)
# ---------------------------------------------------------------------------
class LowTri:
    def __init__(self, m):
        self._m = m
...
class DeepLagrangianNetwork(nn.Module):
    ...
    Inputs to forward(): q, qd, qdd  each (B, n_dof)
    Outputs:             tau_pred (B, n_dof),  dEdt (B,)

    Use inv_dyn(q, qd, qdd) -> tau_pred       for inverse dynamics
    Use for_dyn(q, qd, tau) -> qdd_pred       for forward dynamics (rollout)
    Use energy(q, qd) -> E                    for total energy T+V
    """
