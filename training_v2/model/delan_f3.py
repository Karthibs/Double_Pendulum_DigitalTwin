"""
Deep Lagrangian Networks with a NEURAL NETWORK friction model.
"""
import numpy as np
import torch
...
# NN friction module
# ---------------------------------------------------------------------------
class NNFriction(nn.Module):
    """Small MLP friction model: F(qd) : R^n_dof -> R^n_dof."""
    def __init__(self, n_dof, hidden=32, depth=2,
                 input_scale=10.0, output_scale_init=0.01):
        super().__init__()
        self.n_dof = n_dof
        ...
        for _ in range(depth):
            layers.append(nn.Linear(in_dim, hidden, bias=True))
            layers.append(nn.Tanh())
            in_dim = hidden
        layers.append(nn.Linear(in_dim, n_dof, bias=False))   # F(0) = 0
        self.net = nn.Sequential(*layers)

        # Output scale (log-parameterized so it stays positive)
        self.log_output_scale = nn.Parameter(
            torch.log(torch.tensor(float(output_scale_init)))
        )

        # Initialize weights small to keep early friction quiet
    ...
    def forward(self, qd):
        ...
        z = qd / self.input_scale
        return scale * self.net(z)


class DeepLagrangianNetwork(nn.Module):
    """DeLaN with neural-network friction model."""

    def __init__(self, n_dof, learn_friction=True,
                 friction_hidden=32, friction_depth=2,
                 friction_input_scale=10.0,
                 friction_output_scale_init=0.01,
                 **kwargs):
        super().__init__()
        ...
        non_linearity  = kwargs.get("activation", "SoftPlus")
        ...
        if self._w_init == "xavier_normal":
            ...
            raise ValueError("Only xavier_normal supported in this variant")
        ...
        if learn_friction:
            self.friction_net = NNFriction(
                n_dof=n_dof,
                hidden=friction_hidden,
                depth=friction_depth,
                input_scale=friction_input_scale,
                output_scale_init=friction_output_scale_init,
            )
    ...
    def friction(self, qd):
        ...
        return self.friction_net(qd)

    # ---- Diagnostics for training-loop penalties ----
    def friction_diagnostics(self, qd):
        if not self.learn_friction:
            zero = torch.zeros((), device=qd.device)
            return {'dissipativity_violation': zero,
                    'friction_l2': zero,
                    'antisymmetry_violation': zero}

        Fp = self.friction_net(qd)
        Fn = self.friction_net(-qd)

        power = (qd * Fp).sum(dim=-1)           # qd^T F(qd)
        # Dissipativity: friction should oppose motion, so qd^T F(qd) >= 0.
        # Penalize cases where it's negative.
        dissipativity_violation = torch.clamp(-power, min=0.0).mean()

        friction_l2 = (Fp ** 2).sum(dim=-1).mean()
        antisymmetry_violation = ((Fp + Fn) ** 2).sum(dim=-1).mean()

        return {
            'dissipativity_violation': dissipativity_violation,
            'friction_l2': friction_l2,
            'antisymmetry_violation': antisymmetry_violation,
        }

    # ---- Core dynamics (identical to delan.py) ----
