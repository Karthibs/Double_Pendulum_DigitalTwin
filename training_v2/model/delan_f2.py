...
Friction model
--------------
Following Lutter & Peters 2023, the friction torque on
joint i is the combined Stribeck model:

    F_i(q_dot_i) = ( tau_c_i + tau_s_i * exp(-q_dot_i^2 / nu_i) )
                   * sign(q_dot_i)
                 + d_i * q_dot_i
...
class LagrangianLayer(nn.Module):
    ...
    def __init__(self, input_size, n_dof, activation="ReLu"):
        ...
        if activation == "ReLu":
            ...
            raise ValueError(
                f"Activation '{activation}' not in [ReLu, SoftPlus, Sine, Linear]"
            )
...
class DeepLagrangianNetwork(nn.Module):
    ...
    def __init__(self, n_dof, learn_friction=False, friction_sharpness=100.0, **kwargs):
        super().__init__()
        ...
        # F_i(qd) = (tau_c + tau_s * exp(-qd^2 / nu)) * tanh(k*qd)  +  d * qd
        # All four parameters are log-parameterized so they stay > 0
        # (friction must dissipate energy, never inject it).
        # `friction_sharpness` (k) is a FIXED hyperparameter, not learned.
        self.learn_friction = learn_friction
        self.friction_sharpness = float(friction_sharpness)
        if learn_friction:
            # Reasonable defaults for robot joints, in S.I. units
            self.log_tau_c = nn.Parameter(torch.log(torch.full((n_dof,), 1e-2)))  # Coulomb  [N.m]
            self.log_tau_s = nn.Parameter(torch.log(torch.full((n_dof,), 1e-2)))  # stiction [N.m]
            self.log_nu    = nn.Parameter(torch.log(torch.full((n_dof,), 1e-1)))  # stiction width [(rad/s)^2]
            self.log_d     = nn.Parameter(torch.log(torch.full((n_dof,), 1e-2)))  # viscous  [N.m.s/rad]
    ...
    def friction(self, qd):
        ...
        tau_c = torch.exp(self.log_tau_c)
        tau_s = torch.exp(self.log_tau_s)
        nu    = torch.exp(self.log_nu)
        d     = torch.exp(self.log_d)

        # Smooth, differentiable approximation of sign(qd) that saturates at +/- 1.
        sign_smooth = torch.tanh(self.friction_sharpness * qd)

        # Coulomb + stiction (the stiction term decays away from qd = 0)
        coulomb_stiction = (tau_c + tau_s * torch.exp(-qd * qd / nu)) * sign_smooth

        # Viscous
        viscous = d * qd

        return coulomb_stiction + viscous

    # ---- Core dynamics computation (Lutter's _dyn_model) ----
    def _dyn_model(self, q, qd, qdd):
        ...
        # Coriolis & centrifugal:  c = Hdt q_dot - 0.5 d(q_dot^T H q_dot)/dq
        ...
        # Inverse dynamics: tau = H q_ddot + c + g (+ friction, if enabled)
    ...
    # ---- Friction parameter accessors (read-only convenience) ----
    def friction_params(self):
        """Return identified Stribeck parameters as a dict of (n_dof,) tensors.

        Useful for inspecting / logging after training. Returns None if
        friction learning is disabled.
        """
        if not self.learn_friction:
            return None
        with torch.no_grad():
            return {
                "tau_c": torch.exp(self.log_tau_c).cpu(),
                "tau_s": torch.exp(self.log_tau_s).cpu(),
                "nu":    torch.exp(self.log_nu).cpu(),
                "d":     torch.exp(self.log_d).cpu(),
                "sharpness": self.friction_sharpness,
            }
